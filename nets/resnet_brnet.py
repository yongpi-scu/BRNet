...
__all__ = ['resnet50_brnet']

model_urls = {
    'resnet50': '/root/workspace/res/ckpt/pytorch/resnet/resnet50-19c8e357.pth',
}
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, color_channels=3, num_classes=1000, zero_init_residual=False,
        ...
        self.conv1 = MultiScale(color_channels, self.inplanes)
...
def _load_pretrain(net, model_name, exclusions):
...
def _resnet(arch, block, layers, pretrained, color_channels, **kwargs):
    model = ResNet(block, layers, color_channels, **kwargs)
    if pretrained:
        exclusions = ['fc', 'conv1', "bn1"]
        model = _load_pretrain(model, arch, exclusions)
    return model

def resnet50_brnet(pretrained=False, color_channels=3, **kwargs):
    return _resnet('resnet50', Bottleneck, [3, 4, 6, 3], pretrained, color_channels=color_channels, **kwargs)